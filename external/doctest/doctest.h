...
struct DOCTEST_INTERFACE Result { // NOLINT(*-member-init)
    ...
    Result() = default; // TODOD: Why do we need this? (To remove NOLINT)
...
// TODOD: figure out if this is indeed necessary/correct - seems like either there still
...
bool parseIntOption(int argc, const char *const *argv, const char *pattern, optionType type, int &res) {
    ...
    if (type) {
        // integer
        // TODOD: change this to use std::stoi or something else! currently it uses undefined
...
int Context::run() {
    ...
    // TODOD: check if there is nothing in reporters_currently_used
    ...
    if (getContextOptions()->no_path_in_filenames) {
        ...
        for (String::size_type pos = 0U; pos < prefixes.size(); ++pos) {
            ...
            if (prefix_size > longest_match) {
                // TODOD under DOCTEST_MSVC: does the comparison need strnicmp() to work with drive
...
void XmlReporter::test_run_start() {
    ...
    // only the consequential ones (TODOD: filters)
